...
const RootComponent = () => {
  ...
  const [currentVideo, setCurrentVideo] = useState(`${process.env.PUBLIC_URL}/videos/video2.mp4`);
    ...
    const checkNewYear = () => {
      ...
      if (isNewYear) {
        ...
        if (isInFirecrackerTime) {
          ...
          setCurrentVideo(`${process.env.PUBLIC_URL}/videos/video2.mp4`);
        }
      } else {
        setCurrentVideo(`${process.env.PUBLIC_URL}/videos/video2.mp4`);
      }
  ...
  return (
    <React.StrictMode>
      <div
        ...
        onClick={handleUnmute} // Unmute on user interaction
      >
        {/* Background video */}
        <video
          ref={videoRef}
          ...
          loop
          playsInline
          muted={isMuted} // Initially muted for autoplay compliance
          style={{
            position: 'absolute',
            top: 0,
            left: 0,
